...
export function ShareModal({ entry, onClose }: ShareModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-bg border-2 border-ink/15 shadow-lg flex flex-col max-h-[90vh] relative">
        <button 
          onClick={onClose} 
          className="absolute -top-4 -right-4 w-10 h-10 bg-accent text-bg border-2 border-ink/15 flex items-center justify-center hover:scale-110 transition-transform z-10"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="p-4 md:p-6 overflow-y-auto custom-scrollbar flex-1 flex flex-col items-center">
          {isDownloading && !ogUrl ? (
            <div className="flex flex-col items-center justify-center flex-1 h-64 space-y-4">
               <div className="w-8 h-8 border-2 border-ink/20 border-t-transparent rounded-full animate-spin"></div>
            ...
            <div className="w-full">
              {ogUrl && (
                <div className="relative mb-6">
                  <img src={ogUrl} alt="Diary Share Card" className={`w-full h-auto border-2 border-ink/15 shadow-sm transition-opacity ${isDownloading ? 'opacity-50' : 'opacity-100'}`} />
                  {isDownloading && (
              ...
              <div className="w-full flex flex-col gap-4">
                <div className="w-full flex items-center justify-between border-2 border-ink/15 p-3 bg-white">
                  <span className="font-mono text-xs font-bold uppercase">Background Color</span>
                  <input 
                    type="color" 
                    value={bgColor} 
                    onChange={(e) => setBgColor(e.target.value)}
                    className="w-8 h-8 rounded-full border-2 border-ink/15 cursor-pointer p-0 overflow-hidden"
                  />
                </div>
                <a 
                  href={ogUrl || '#'} 
                  download={`exit_${entry.type}_${new Date().getTime()}.png`}
                  className={`w-full flex items-center justify-center gap-2 h-14 bg-brand text-ink border-2 border-ink/15 font-heading uppercase text-sm shadow-sm transition-transform ${isDownloading ? 'opacity-50 pointer-events-none' : 'hover:-translate-y-1'}`}
                >
